...
if __name__ == '__main__':
    ...
    if not args.is_regression:
        ...
        #FIXME
        print('Scores for regression not available yet..')
        #breakpoint()
    ...
    if not args.is_regression:
        ...
        #FIXME
        print('Scores for regression not available yet...')
        #centered_score = all_scores #[0] - (1/2) #1/le'(classes) or just 1 / 2 because I split median?
        #scale_factor = torch.quantile(
        #centered_score.abs(), args.score_threshold) * 2
    ...
    for slide_url in (progress := tqdm(args.slide_urls, leave=False)):
        ...
        if not args.is_regression:
            ...
            #FIXME\
            print('Score map for regression NA yet...')
            #scaled_score_map = (
